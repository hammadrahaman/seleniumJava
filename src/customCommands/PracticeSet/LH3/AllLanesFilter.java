...
public class AllLanesFilter {
    ...
    public AllLanesFilter(WebDriver driver) {
        this.driver = driver;
    ...
    @FindBy(xpath = "("+lanes + "//*[@aria-label='Confirm Projection'])[position()='1']")
    ...
    public void iconFilters(String column) {
        System.out.println("\n-----------------------");
        System.out.println("Processing column: " + column);
        WebElement element = getIconFilters(column);
        if (element != null) {
            validateTheIconFilters(element, column);
        } else {
            System.out.println("Invalid column: " + column);
        }
    }

    private WebElement getIconFilters(String column) {
        switch (column) {
    ...
    public void iconFilterApplyFalse(String column, WebElement element) {
        ...
        try {
            if (!element.isDisplayed()) {
                System.out.println("Element is NOT displayed for False which is expected");
                clearFilter();
            }
        } catch (NoSuchElementException e) {
    ...
    public void validateTheIconFilters(WebElement element, String column) {
        try {
            wait4s();
            filterIcon.click();
            wait1s();
            ...
            System.out.println("No result found for " + column+ " as value true");
            iconFilterApplyFalse(column, element);
        }
    }
}
